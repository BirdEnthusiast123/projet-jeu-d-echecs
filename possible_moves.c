...
void parse_fen_string(Game *g, char *fen_string)
{
	...
	if (fen_string[i] != '-')
	{
		g->en_pass_x = fen_string[i] - 'a';
		i++;
		g->en_pass_y = parse_int_char(fen_string[i]) - 1;
		i++;
	}
	else
	{
...
int main()
{
	...
	char* fen = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR b Qq h8 50 100";
